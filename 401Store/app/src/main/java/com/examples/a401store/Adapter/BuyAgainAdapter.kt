...
import com.examples.a401store.databinding.BuyAgainItemBinding
...
class BuyAgainAdapter(
    ...
    inner class BuyAgainViewHolder(private val binding: BuyAgainItemBinding) :
        ...
        fun bind(foodName: String, foodPrice: String, foodImage: String) {
            binding.txtBuyAgainFoodName.text = foodName
            binding.txtBuyAgainPrice.text = foodPrice
            val uriString = foodImage
            val uri = Uri.parse(uriString)
            Glide.with(requireContext).load(uri).into(binding.imgBuyAgainFood)
        }
